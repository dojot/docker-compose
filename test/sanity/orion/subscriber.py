...
def parseRFID(data):
    ...
    for d in jdata['contextResponses']:
        ret = {}
        for attr in d['contextElement']['attributes']:
            ret[attr['name']] = attr['value']
        return ret

def handleEvent(data):
    jdata = json.loads(data)
    event = parseRFID(data)
    print event
...
def setThreshold():
    global threshold
    print "about to set threshold"
    thr = json.loads(request.data)
    print thr
    for n in thr:
        print n
        if n['type'] == 'switch':
            newThr = float(n['rules'][0]['v'])
            print "setting new threshold to %f" % newThr
            threshold = newThr
...
def record():
    ...
    if ((request.data is not None) and (len(request.data) != 0)):
        ...
        # socketio.emit('rfid', parseRFID(request.data))
        # socketio.emit('rfid', request.data)
        if pretty == True:
